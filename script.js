// =============================================================================
// ProfitPerPlate - Firebase Integration + MANUAL SAVE SYSTEM
// =============================================================================
// Replaced: Supabase client calls with Firebase Firestore/Auth logic
...
document.addEventListener('DOMContentLoaded', function() {
    // Force immediate check if firebase-config already loaded
    if (window.supabaseClient && window.supabaseClient.initialized) {
        setupMonetizationTracking();
    } else {
        // Fallback wait
...
// Defensive helper functions to avoid ReferenceErrors and provide light validation.
// Place this file before the end of script.js override block (or include it before script.js in HTML).

(function () {
  // Helper to check auth state (supports both firebase and the app's supabaseClient patterns)
  function _isAuthenticated() {
    try {
      ...
      if (window.supabaseClient && typeof window.supabaseClient.getCurrentUser === 'function') {
        return !!window.supabaseClient.getCurrentUser();
      }
      // generic fallback: app exposes current user via window.supabaseClient.currentUser in the script
      if (window.supabaseClient && window.supabaseClient.currentUser) {
        return !!window.supabaseClient.currentUser;
      }
      // If we cannot determine, assume not authenticated to be conservative? — here we return false to allow UI to prompt auth
      return false;
    } catch (err) {
  ...
  function _maybeShowAuthPrompt() {
    try {
      ...
      if (typeof window.Mt === 'function') {
        Mt(); // fallback naming used in script.js for opening auth modal
        return;
      }
      // lightweight notification fallback
      window.showNotification?.('Please log in to continue', 'warning');
    } catch (err) {
      console.warn('Failed to show auth prompt:', err);
    }
  }

  // Generic validator factory used by the three specific validators below
  function makeValidator({ monetizationCheckName, monetizationFeatureKey, requireAuth = true }) {
    return function () {
      try {
        // 1) monetization check (if present)
        if (window.monetization && typeof window.monetization[monetizationCheckName] === 'function') {
          try {
            if (!window.monetization[monetizationCheckName]()) {
              // If monetization says no, show upgrade UI if available
              window.monetization.showUpgradeModal?.(monetizationFeatureKey || '');
              return false;
            }
          } catch (e) {
            // If monetization check throws, log and continue (do not block)
            console.warn('Monetization check failed:', e);
          }
        }

        // 2) auth check (optional)
        if (requireAuth) {
          const ok = _isAuthenticated();
          if (!ok) {
            // ask user to sign in
            _maybeShowAuthPrompt();
            return false;
          }
        }

        // All checks passed (or none applied) => allow action
        return true;
      } catch (err) {
        // Defensive fallback: do not block the user due to validator error
        console.warn('validate helper exception (allowing action):', err);
        return true;
      }
    };
  }

  // Export the three validators used by script.js
  window.validateBeforeAddingRawMaterial = makeValidator({
    monetizationCheckName: 'canAddRawMaterial',
    monetizationFeatureKey: 'raw_materials',
    requireAuth: true
  });

  window.validateBeforeAddingDirectLabor = makeValidator({
    monetizationCheckName: 'canAddDirectLabor',
    monetizationFeatureKey: 'direct_labor',
    requireAuth: true
  });

  window.validateBeforeAddingMainRecipe = makeValidator({
    monetizationCheckName: 'canAddMainRecipe',
    monetizationFeatureKey: 'main_recipes',
    requireAuth: true
  });

  // Defensive no-op exports if later code checks existence differently
  window.__validateHelpersInstalled = true;
})();
